...
d3.print(std::cout);
  /*{
    ...
    Point2D sum2D;
    Point3D sum3D, * ptr = nullptr;

    for (int i = 0; i < points.size(); i++)
    {
      ptr = dynamic_cast<Point3D*>(points[i]);
      if (ptr)
        sum3D = sum3D + *ptr;
      else
        sum2D = sum2D + *(points[i]);
    }
    for (vector<Point2D*>::iterator i = points.begin(); i < points.end(); i++)
    {
      delete* i;
    }
    sum2D.print(std::cout);
    std::cout << "\n";
    sum3D.print(std::cout);
    std::cout << "\n";
  }
  system("pause");*/
  return 0;
